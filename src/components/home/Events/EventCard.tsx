...
export default function EventCard({ className, alt = 1, title = "Event title", details = "Event Description Lorem ipsum dolor sit amet consectetur adipisicing elit", img = "ImagePlaceholder.webp", date = Date().slice(0, 15), location = "Agartala", badge = undefined, listitem = false }: LocationBoxProps) {
    return (
        <figure className={cn("relative border border-gray-200 rounded-lg cursor-pointer", className, {
            ...
            <div className={cn("overflow-hidden", {
                ...
                <Image src={`/EventImages/${img}`} alt={title.split(" ").map(w => w[0]).join("") + alt} width={200} height={200} className={cn("w-fit object-cover",
                    {
